...
# ==================== SELECT MENU CHO HYPESQUAD ====================
class HypeSquadSelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label="🛡️ Bravery (Tím)", value="1", description="Dũng cảm - Huy hiệu màu Tím", emoji="🛡️"),
            discord.SelectOption(label="💡 Brilliance (Đỏ)", value="2", description="Thông thái - Huy hiệu màu Đỏ", emoji="💡"),
            discord.SelectOption(label="⚖️ Balance (Xanh)", value="3", description="Công bằng - Huy hiệu màu Xanh", emoji="⚖️"),
        ]
        super().__init__(placeholder="🏠 Chọn màu nhà bạn muốn tham gia...", options=options)

    async def callback(self, interaction: discord.Interaction):
        await interaction.response.send_modal(HypeSquadTokenModal(self.values[0]))

class HypeSquadTokenModal(discord.ui.Modal, title="🏠 LẤY HUY HIỆU HYPESQUAD"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        token = self.children[0].value.strip()
        house = self.house_id
        house_name = {"1": "Bravery (Tím) 🛡️", "2": "Brilliance (Đỏ) 💡", "3": "Balance (Xanh) ⚖️"}[house]
...
class MainSelect(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label="🏠 Lấy huy hiệu HypeSquad", value="hypesquad", description="Chọn màu nhà để nhận huy hiệu", emoji="🏠"),
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.values[0] == "quest":
            ...
            view.add_item(HypeSquadSelect())
            await interaction.response.send_message("🏠 **Chọn màu nhà HypeSquad:**", view=view, ephemeral=True)
...
@bot.tree.command(name="menu", description="📋 Hiển thị menu chính của bot")
async def cmd_menu(interaction: discord.Interaction):
    ...
    embed.add_field(
        ...
        value="[Click vào đây để xem video hướng dẫn lấy token trên Điện Thoại](https://files.catbox.moe/lsl6pa.mp4)",
        inline=False
    )
    embed.set_footer(text="Xoài Xanh • Chọn từ menu thả xuống bên dưới")
    embed.set_thumbnail(url="https://cdn.discordapp.com/emojis/1302178935064367125.png")
    
    view = MainView()
    
    await interaction.response.send_message(embed=embed, view=view, ephemeral=False)

@bot.tree.command(name="cancel", description="🛑 Dừng tiến trình đang chạy")
async def cmd_cancel(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("🛑 **Đã dừng tiến trình thành công!**", ephemeral=True)

@bot.tree.command(name="status", description="📊 Xem trạng thái tiến trình")
async def cmd_status(interaction: discord.Interaction):
    if interaction.user.id not in active_sessions:
        await interaction.response.send_message("📊 Không có tiến trình nào đang chạy!", ephemeral=True)
        return
    completer = active_sessions[interaction.user.id]["completer"]
    embed = discord.Embed(title="📊 TRẠNG THÁI TIẾN TRÌNH", color=0x2b2d31)
    embed.add_field(name="✅ Đã hoàn thành:", value=f"`{len(completer.completed_ids)}` nhiệm vụ", inline=True)
    embed.add_field(name="🏆 Tổng số:", value=f"`{completer.todo_initial}` nhiệm vụ", inline=True)
